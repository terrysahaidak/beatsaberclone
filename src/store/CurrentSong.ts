...
export class CurrentSongStore {
  ...
  onMapPlay() {
    this.root.setState('map-playing');

    this.onPlay?.();
  }

  onMapReady() {
    this.root.setState('map-loaded');
  }

  onMapEnd() {
    this.root.setState('map-end');
  }
  ...
  onCollision(_block: BlockModel, shouldCount: boolean, hand: 'left' | 'right') {
    ...
    this.root.sabers.onHitVibration(hand);
  }
  ...
  calculateCollisions(hand: 'left' | 'right') {
    ...
    const boundingBox = this.root.sabers.getBoundingBox(hand);
